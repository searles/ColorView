...
import at.searles.colorview.utils.RybColors;
import at.searles.colorview.utils.RgbCommons;
...
public class ColorView extends View {
	...
	public void setColor(int rgb) {
		float[] rgbf = RgbCommons.int2rgb(rgb, new float[3]);
	...
	public int getColor() {
		float r = RybColors.red(hue);
		float g = RybColors.green(hue);
		float b = RybColors.blue(hue);
	...
	class HueCircle {
		...
		void initGradients() {
			int[] colors = new int[RybColors.colorSegmentsCount() + 1];

			for(int i = 0; i < RybColors.colorSegmentsCount(); ++i) {
				float hue = ((float) i) / (float) RybColors.colorSegmentsCount();
				float r = RybColors.red(hue);
				float g = RybColors.green(hue);
				float b = RybColors.blue(hue);
	...
	class Triangle {
		...
		void updateGeometry() {
			...
			// Color of triangle
			LinearGradient cbGradient = new LinearGradient(x(ax), y(ay), x(-ax / 2.f), y(-ay / 2.f), RybColors.color(hue), Color.BLACK, Shader.TileMode.CLAMP);
